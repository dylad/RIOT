...
#    define GPIO_PIN(x, y)  (((gpio_t)(&PORT_IOBUS_SEC_REGS->Group[x])) | y)
#  else /* Use IOBUS access when available */
#    define GPIO_PIN(x, y)  (((gpio_t)(&PORT_IOBUS_REGS->Group[x])) | y)
#  endif /* PORT_IOBUS_SEC */
#else
#  ifdef PORT_SEC
#    define GPIO_PIN(x, y)  (((gpio_t)(&PORT_SEC_REGS->Group[x])) | y)
#  else
#    define GPIO_PIN(x, y)  (((gpio_t)(&PORT_REGS->Group[x])) | y)
#  endif /* PORT_IOBUS_SEC */
#endif
 ...
 */
typedef struct {
    sercom_registers_t *dev;  /**< pointer to the used UART device */
...
#ifdef MCLK
    volatile uint32_t *mclk;    /**< Pointer to MCLK_REGS->APBxMASK */
 ...
 */
typedef struct {
    sercom_i2cm_registers_t *dev;   /**< pointer to the used I2C device */
 ...
 */
typedef struct {
    tc_count32_registers_t *dev;    /**< pointer to the used Timer device */
...
static inline void sam0_set_voltage_regulator(sam0_supc_t src)
{
    ...
    if (src == SAM0_VREG_BUCK) {
        SUPC_REGS->VREG |= (1 << SUPC_VREG_SEL_Pos);
    }
    else {
        SUPC_REGS->VREG &= ~(1 << SUPC_VREG_SEL_Pos);
    }
    while (!(SUPC_REGS->STATUS & SUPC_STATUS_VREGRDY)) {}
#else
    (void) src;
    assert(0);
#endif
...
#endif
#ifdef SERCOM1
    if (sercom == SERCOM1_REGS) {
        return 1;
    }
#endif
#ifdef SERCOM2
    if (sercom == SERCOM2_REGS) {
        return 2;
    }
#endif
#ifdef SERCOM3
    if (sercom == SERCOM3_REGS) {
        return 3;
    }
#endif
#ifdef SERCOM4
...
#endif
#ifdef SERCOM5
    if (sercom == SERCOM5_REGS) {
        return 5;
    }
#endif
#ifdef SERCOM6
    if (sercom == SERCOM6_REGS) {
        return 6;
    }
#endif
#ifdef SERCOM7
    if (sercom == SERCOM7_REGS) {
        return 7;
    }
#endif
...
static inline void sercom_clk_en(void *sercom)
{
    ...
    PM_REGS->PM_APBCMASK |= (PM_APBCMASK_SERCOM0 << id);
#elif defined (CPU_COMMON_SAMD5X)
    if (id < 2) {
        MCLK_REGS->MCLK_APBAMASK |= (1 << (id + 12));
    } else if (id < 4) {
        MCLK_REGS->MCLK_APBBMASK |= (1 << (id + 7));
    } else {
        MCLK_REGS->MCLK_APBDMASK |= (1 << (id - 4));
    }
#else
    if (id < 5) {
        MCLK_REGS->MCLK_APBCMASK |= (MCLK_APBCMASK_SERCOM0 << id);
    }
#  if defined(CPU_COMMON_SAML21)
    else {
        MCLK_REGS->MCLK_APBDMASK |= (MCLK_APBDMASK_SERCOM5);
    }
...
#if defined(CPU_COMMON_SAMD21)
    GCLK_REGS->GCLK_CLKCTRL = (GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN(gclk) |
                         (SERCOM0_GCLK_ID_CORE + id));
    while (GCLK_REGS->GCLK_STATUS & GCLK_STATUS_SYNCBUSY) {}
#elif defined(CPU_COMMON_SAMD5X)
    GCLK_REGS->GCLK_PCHCTRL[_sercom_gclk_id_core(id)] = (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN(gclk));
#else
    if (id < 5) {
        GCLK_REGS->GCLK_PCHCTRL[SERCOM0_GCLK_ID_CORE + id] = (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN(gclk));
    }
#  if defined(CPU_COMMON_SAML21)
    else {
        GCLK_REGS->GCLK_PCHCTRL[SERCOM5_GCLK_ID_CORE] = (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN(gclk));
    }
#  endif /* CPU_COMMON_SAML21 */
#endif
}
...
static inline bool cpu_woke_from_backup(void)
{
    ...
    return RSTC_REGS->RSTC_RCAUSE & RSTC_RCAUSE_BACKUP;
#else
    return false;
#endif
}
 ...
 *
 *          Will only write bits 1 _REGS-> 0. To reset bits to 1, call @see sam0_flashpage_aux_reset
