...
#define ADXL362_SELF_TEST_SLEEP            (100 * US_PER_MS)
...
static int adxl362_read_regs(const adxl362_t *dev, uint8_t addr, uint8_t *buf, size_t count)
{
    ...
    spi_acquire(dev->spi, dev->cs, dev->spi_mode, dev->spi_clk);
    int res;
    res = spi_transfer_byte(dev->spi, dev->cs, true, ADXL362_CMD_READ_REG);
    if (res < 0) {
        DEBUG("adxl362_read_regs: SPI ERR!\n");
        return res;
    }
    spi_transfer_regs(dev->spi, dev->cs, addr, NULL, (char *)buf, count);
...
{
    spi_acquire(dev->spi, dev->cs, dev->spi_mode, dev->spi_clk);
    spi_transfer_byte(dev->spi, dev->cs, true, ADXL362_CMD_WRITE_REG);
    spi_transfer_reg(dev->spi, dev->cs, addr, val);
    spi_release(dev->spi);
...
static int adxl362_read_fifo(const adxl362_t *dev, uint8_t *buf, size_t bufsize)
{
    ...
    adxl362_read_regs(dev, ADXL362_REG_FIFO_ENTRIES_L, tmp.u8, sizeof(tmp.u8));
    ...
    spi_acquire(dev->spi, dev->cs, dev->spi_mode, dev->spi_clk);
    spi_transfer_regs(dev->spi, dev->cs, ADXL362_CMD_READ_FIFO, NULL, (char *)buf, nentries * sizeof(uint16_t));
    spi_release(dev->spi);
